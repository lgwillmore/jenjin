...
public class GifReader {
	...
	protected void setPixels() {
		...
		// Pixmap.createPixmap(dest, width, height, JJConfig.ARGB_4444);
	}
	...
	public GifTexture getTexture() {
		...
		System.out.println("Texture regions in gif: "+texReg.size);
		GifTexture gifTexture = new GifTexture(texture,texReg);
		return gifTexture;
	}

	public GifTexture readGifTexture(InputStream is) {
		read(is);
		if(err()){
			throw new RuntimeException("Could not read Gif. Status: "+status);
		}
		return getTexture();
	}
}